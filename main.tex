...
std::vector<int> Erat(int n) {
    ...
    for (size_t jndex = 0; jndex <       primes.size() && primes[jndex] <
        erat[jndex] && index * primes[jndex] <= n; ++jndex) {
...
% \textbf{Стек с произвольной ассоциативной операцией:}
...
%         int oper_element = (data_.empty() ? element :
%         operation_(data_.top().second, element));
...
% \begin{definition}{Наименьшее общее кратное (НОК)}{}
%   Всякое целое, кратное всем данным числам, называется их общим кратным. Наименьшее положительное общее кратное обозначается $[a, b, \ldots, l]$ или $\text{lcm}(a,b,\ldots,l)$.
% \end{definition}
% \begin{implementation}{Алгоритм поиска НОК}{}
%   \begin{lstlisting}[language=c++]
% int lcm(int a, int b) {
%     return a * b / gcd(a, b);
% }
...
% \textbf{Связь НОД и НОК:}
% \[
%   \gcd(a,b) \cdot \text{lcm}(a,b) = a \cdot b
% \]


% \begin{theorem}{Эквивалентные условия сравнимости}{}
%   Сравнимость чисел $a$ и $b$ по модулю $m$ эквивалентна:
%   \begin{enumerate}
%     \item Возможности представить $a$ как $a = b + mt$.
%     \item Делимости $a - b$ на $m$.
%   \end{enumerate}
% \end{theorem}

% \textbf{Свойства сравнений:}
% \begin{enumerate}
%   \item Два числа, сравнимые с третьим, сравнимы между собою.
%   \item Сравнения можно почленно складывать.
%   \item Слагаемое, стоящее в какой-либо части сравнения, можно переносить в другую часть, переменив знак на обратный.
%   \item Сравнения можно почленно перемножать.
%   \item Обе части сравнения можно возвести в одну степень.
%   \item К каждой части сравнения можно добавить число, кратное модулю.
%   \item Обе части сравнения можно умножить на одно и то же целое.
% \end{enumerate}

% \begin{definition}{Полная система вычетов}{}
%   Набор из $m$ чисел, дающих все возможные остатки при делении на $m$. Обычно это $\{0, 1, 2, \ldots, m-1\}$.
% \end{definition}

% \begin{definition}{Приведённая система вычетов}{}
%   Только те числа из полной системы, которые взаимно просты с $m$. Таких чисел $\varphi(m)$.
% \end{definition}
...
% \textbf{Цель:} Обычный алгоритм Евклида находит НОД. Расширенный находит ещё и коэффициенты $x, y$ такие, что:
% \[
%   a \cdot x + b \cdot y = \gcd(a, b)
% \]
% \begin{implementation}{Реализация расишеренного алгоритма Евклида}{}
%   \begin{lstlisting}[language=c++]
% std::pair<int,std::pair<int,int>> Evcl(int a, int b) {
%     if (a < b) std::swap(a, b);
%     if (!b) return {a, {1, 0}};
%     auto [ans, x, y] = Ecvl(b, a % b);
%     return {ans, {y, x - a / b * y}};
% }
% \end{lstlisting}
% \end{implementation}
% Это называется \textbf{тождество Безу}.

% \begin{algorithm}{Расширенный алгоритм Евклида}{}
%   Идём «назад» по шагам обычного алгоритма Евклида, выражая каждый остаток через предыдущие.

%   \textbf{Пример:} $\gcd(30, 18) = 6$.
...
%   \begin{align*}
%     6 & = 18 - 12 \cdot 1                \\
%     6 & = 18 - (30 - 18 \cdot 1) \cdot 1 \\
%     6 & = 18 - 30 + 18                   \\
%     6 & = 18 \cdot 2 - 30 \cdot 1
%   \end{align*}

%   Итак, $x = -1, y = 2$: $30 \cdot (-1) + 18 \cdot 2 = -30 + 36 = 6$.
% \end{algorithm}

% \subsection{Поиск обратного в $\mathbb{Z}_m$}

% \begin{definition}{Обратный элемент}{}
%   Число $a^{-1}$ такое, что $a \cdot a^{-1} \equiv 1 \pmod{m}$. Существует только если $\gcd(a, m) = 1$.
% \end{definition}

% \textbf{Метод 1: Через теорему Эйлера}

% Из теоремы Эйлера: $a^{\varphi(m)} \equiv 1 \pmod{m}$.

% Значит: $a \cdot a^{\varphi(m)-1} \equiv 1 \pmod{m}$.

% Отсюда: $a^{-1} \equiv a^{\varphi(m)-1} \pmod{m}$.

% \textbf{Метод 2: Через расширенный алгоритм Евклида}

% Находим $x, y$: $a \cdot x + m \cdot y = 1$.

% Тогда $a \cdot x \equiv 1 \pmod{m}$, значит $x$ и есть обратный!

% \textbf{Какой метод лучше?} РАЕ быстрее, особенно для больших чисел.

% \subsection{Китайская теорема об остатках}

% \begin{theorem}{Китайская теорема об остатках (КТО)}{}
%   Если модули $m_1, m_2, \ldots, m_k$ попарно взаимно просты, то система сравнений:
%   \begin{align*}
...
% \subsection{Постановка задачи сортировки. Квадратичные сортировки}
...
% \textbf{Три квадратичные сортировки:}

% \begin{enumerate}
%   \item \textbf{Сортировка пузырьком:} Выполним $n$ итераций, в результате каждой из которых самый большой элемент «всплывает» в конец. В процессе итерации пробегаем из начала в конец и меняем соседние элементы, которые стоят не по порядку.
%         \begin{algorithm}{Сортировка пузырьком}{}
%         \end{algorithm}

%         \begin{implementation}{Расширенный алгоритм Евклида}{}
%           \begin{lstlisting}[language=c++]
% void BubbleSort(std::vector<int>& vec) {
%     for (size_t index = 0; index < vec.size(); ++index) {
%         for (size_t jndex = 0; jndex < vec.size() - 1; ++jndex) {
%             if (vec[jndex] > vec[jndex + 1]) {
%                 std::swap(vec[jndex], vec[jndex + 1]);
%             }
%         }
%     }
% }
...
%   \item \textbf{Сортировка выбором:} На $i$-ой итерации ищем на отрезке $[i, n-1]$ наименьший элемент и меняем его с $i$-ым местами.
%         \begin{implementation}{Сортировка выбором}{}
%           \begin{lstlisting}[language=c++]
% void SelectionSort(std::vector<int>& vec) {
%     for (size_t index = 0; index < vec.size(); ++index) {
%         size_t index_of_minimal_element = index;
%         for (size_t jndex = index; jndex < vec.size(); ++jndex) {
%             if (vec[index_of_minimal_element] > vec[jndex]) {
%                 index_of_minimal_element = jndex;
%             }
%         }
%         std::swap(vec[index],   vec[index_of_minimal_element]);
%     }
% }
%     \end{lstlisting}
%         \end{implementation}

%   \item \textbf{Сортировка вставками:} На $i$-ой итерации ищем, куда вставить элемент на отрезке $[0, i]$, таким образом отрезок $[0, i]$ всегда отсортирован.
%         \begin{implementation}{Сортировка встаавками}{}
%           \begin{lstlisting}[language=c++]
% void InsertionSort(std::vector<int>& vec) {
%     for (size_t index = 1; index < vec.size(); ++index) {
%         int element = vec[index];
%         size_t jndex = index;
%         while (jndex > 0 && vec[jndex - 1] > element) {
%             vec[jndex] = vec[jndex - 1];
%             --jndex;
%         }
%         vec[jndex] = element;
%     }
...
% \textbf{Сравнение квадратичных сортировок:}

% \begin{center}
%   \begin{tabular}{|l|c|c|c|}
%     \hline
%     Сортировка & Время    & Память & Стабильность \\ \hline
%     Пузырьком  & $O(n^2)$ & $O(1)$ & Да           \\ \hline
%     Выбором    & $O(n^2)$ & $O(1)$ & Да           \\ \hline
%     Вставками  & $O(n^2)$ & $O(1)$ & Да           \\ \hline
%   \end{tabular}
% \end{center}

% \subsection{Сортировка Шелла. Время работы}

% \textbf{Идея:} Улучшенная сортировка вставками. Вместо сравнения соседних элементов, сравниваем элементы на расстоянии $h$ (шаг). Постепенно уменьшаем $h$ до 1.

% \textbf{Обоснование:} Один обмен соседей может убрать не более 1 инверсии. Если увеличить шаг, то число возможных убираемых инверсий может быть, очевидно, больше чем 1 (если повезет может быть много).

% \textbf{Время работы (без доказательства):}

% В книге «Искусство программирования», Д. Кнут очень много страниц анализирует сортировку Шелла:
% \begin{itemize}
%   \item Шелл предлагал делать шаги размеров от $n/2, n/4, \ldots, 1$. В итоге это давало сложность $O(n^2)$.
%   \item Кнут, в свою очередь, предложил последовательность, дающую сложность в среднем случае $O(n^{4/3})$, в худшем $O(n^{3/2})$.
%   \item Собственно последовательность, предложенная Кнутом: $t_0 = 1, t_i = 3t_{i-1} + 1$.
% \end{itemize}
% \begin{implementation}{Сортировка Шелла}{}
%   \begin{lstlisting}[language=c++]
% void ShellSort(std::vector<int>& vec) {
%     for (size_t step = vec.size() / 2; step > 1; ++step) {
%         for (size_t index = step; index < vec.size(); ++index) {
%             int element = vec[index];
%             size_t jndex = index;
%             while (jndex / step > 0 && vec[jndex - step] > element) {
%                 vec[jndex] = vec[jndex - step];
%                 jndex -= step;
%             }
%         vec[jndex] = element;
%         }
...
% \subsection{Теорема о нижней границе времени работы сортировки}
...
% \subsection{LSD и MSD}

% \textbf{Идея:} Сортировки, основанные не на сравнениях. Вместо сравнения целых чисел, сортируем их по разрядам (цифрам).

% \textbf{Сортировка подсчётом (Counting Sort):}
...
% \textbf{LSD (Least Significant Digit Sort):}
...
% \textbf{MSD (Most Significant Digit Sort):}

% А если пойти от самого значимого разряда? Тогда мы последними будем сортировать младшие разряды и результат для более важных разрядов может перемешаться...

% Давайте после сортировки разряда рекурсивно вызывать сортировку для последовательностей элементов, которые оказались равными в текущем разряде. Можно сказать, что мы разбили элементы на корзины.

% Такую сортировку часто называют «корзинной сортировкой». Алгоритм требует $O(n + k)$ памяти, и также $O(n + k)$ времени (в случае чисел). Можно также использовать для сортировки строк.

% \begin{implementation}{MSD}{}
%   \begin{lstlisting}[language=c++]
% void MSDSort(std::vector<int>& vec) {
%     int max_element = vec[0];
%     for (size_t index = 0; index < vec.size(); ++index) {
%         max_element = std::max(max_element, vec[index]);
%     }
%     int digit_number = 1;
%     size_t max_size = std::to_string(max_element).size();
%     for (size_t count = 0; count < max_size; ++count) {
%         digit_number *= 10;
%     }
%     MSDSort(vec, digit_number, 0, vec.size());
% }
% void MSDSort(std::vector<int>& vec, int digit_number, size_t left_index,
% size_t right_index) {
%     if (right_index - left_index <= 1) {
%         return;
%     }
...
%     }
%     std::vector<int> count_copy(11);
%     for (size_t count_index = 0; count_index < count_copy.size();
%     ++count_index){
%         count_copy[count_index + 1] = count[count_index];
%     }
%     for (size_t index = left_index; index < right_index; ++index) {
%         int digit = (vec[index] / digit_number) % 10;
%         vec_copy[left_index + count[digit] - 1] = vec[index];
%         --count[digit];
%     }
%     if (digit_number != 1) {
%         digit_number /= 10;
%         for (size_t count_index = 1; count_index < count_copy.size();
%         ++count_index) {
%             MSDSort(vec, digit_number, left_index + count_copy[count_index -
%             1], left_index + count_copy[count_index]);
%         }
%     }
% }
%     \end{lstlisting}
...
% \subsection{Быстрая сортировка. Разбиения: Ломуто, Хоара, Толстое. Среднее время работы}
...
% \textbf{Стратегии выбора pivot:}

% \begin{itemize}
%   \item Неплохой стратегией является выбор случайного элемента.
%   \item Также можно брать медиану трех случайных элементов (или даже 5-ти или 7-ми).
%   \item Также существует детерминированный алгоритм (медиана медиан).
% \end{itemize}

% \textbf{Время работы:}

% \begin{lemma}{Время работы быстрой сортировки}{}
%   Пусть $X$ --- число сравнений, выполняемых за время работы сортировки над $n$-элементным массивом. Тогда время работы быстрой сортировки составляет $O(n + X)$.
% \end{lemma}

% \textbf{Доказательство:} Быстрая сортировка делает не более $n$ вызовов функции partition, который в свою очередь совершает некоторое количество итераций цикла, в каждой из которых происходит сравнение элементов с pivot.

% \begin{lemma}{Среднее время работы}{}
%   При случайном выборе pivot: $\mathbb{E}[X] = O(n \log n)$.
% \end{lemma}

% \subsection{k-ая порядковая статистика. Среднее время работы}
...
%   Алгоритм в среднем работает за $O(n)$ (б/д).
% \end{lemma}
...
% \end{lstlisting}
% \end{implementation}
% \subsection{Бинарная куча. Операции. Построение за O(n)}

% \begin{definition}{АТД (абстрактный тип данных)}{}
%   АТД --- интерфейс, набор операций над объектом.
% \end{definition}

% \begin{definition}{АТД очередь с приоритетом}{}
%   АТД очередь с приоритетом --- абстрактный тип данных, поддерживающий две операции:
%   \begin{itemize}
%     \item Вставить элемент
%     \item Извлечь максимум
%   \end{itemize}
% \end{definition}
...
% \textbf{Доказательство:} Заметим, что на высоте $i$ (листья имеют высоту 1) число вершин не превосходит $\lceil \frac{n}{2^i} \rceil$. Одна операция SiftDown делает $2i$ сравнений, а также высота кучи не превосходит $\lceil \log n \rceil$. Тогда:
% \[
%   T(n) \leq \sum_{i=1}^{\lceil \log n \rceil} \frac{n}{2^i} \cdot 2i = 2n \sum_{i=1}^{\lceil \log n \rceil} \frac{i}{2^i}
% \]

% Отметим: $S = \sum_{i=1}^{\lceil \log n \rceil} \frac{i}{2^i}$. Имеем:
% \[
%   S = \sum_{i=1}^{\lceil \log n \rceil} \left( \frac{1}{2} *\frac{i-1}{2^{i-1}} + \frac{1}{2^i} \right) < \frac{1}{2} \cdot \sum_{i=1}^{\lceil \log n \rceil} \frac{i}{2^i} + 1 = \frac{1}{2}S + 1
% \]

% Тогда, $S < 2$, и в свою очередь: $T(n) \leq 4n = O(n)$.

% \textbf{Слияние:} Для слияния двух куч сложим их элементы в один массив и построим кучу. Будет работать за $O(n)$.
...
% На практике обычно используют смешанные алгоритмы, один из которых сейчас будет представлен, такой подход позволяет «сгладить» недостатки одних сортировок преимуществами других.

% \textbf{Проблемы:}

% \begin{itemize}
%   \item У быстрой сортировки есть проблемные случаи, когда рекурсия слишком глубока и когда она слишком долго работает.
%   \item Квадратичные сортировки работают медленно, однако часто хорошо дружат с кешами и имеют малую константу исполнения.
%   \item Сортировка пирамидой использует очень много обменов в памяти, и не сильно дружит с кешем.
% \end{itemize}

% Гениальное решение предложил Дэвид Мюссер в 1997 году --- почему бы их не объединить в «команду»?

% \textbf{Алгоритм Introsort:}

% \begin{enumerate}
%   \item В качестве базовой используется быстрая сортировка.
%   \item При достижении глубины рекурсии равной $\log n$ (при желании можно умножить на константу в большую или меньшую сторону) --- сортируем оставшуюся часть алгоритмом heapsort.
%   \item Совсем маленькие блоки будем сортировать при помощи сортировки Шелла/Вставками (оптимальный размер блока тоже лучше подобрать экспериментально).
% \end{enumerate}

% Поскольку при достижении лимита рекурсии $O(\log n)$ мы переходим на сортировку кучей, алгоритм гарантированно работает за $O(n \log n)$.

% % ==================== РАЗДЕЛ 6: АМОРТИЗАЦИЯ ====================
% \newpage
% \section{Вопрос 6}

% \subsection{ Линейные контейнеры}

% \begin{definition}{Динамически расширяющийся массив}{}
%   Динамически расширяющийся массив --- массивоподобный абстрактный тип данных, предлагающий следующий интерфейс:
%   \begin{itemize}
%     \item push $n$ --- добавить значение $n$ в конец.
%     \item pop --- удалить значение из конца массива.
%     \item get $i$ --- получить i-й элемент массива.
%   \end{itemize}
% \end{definition}
% \begin{implementation}{Динамически расширяющийся массив}{}
%   \begin{lstlisting}[language=c++]
% #include <vector>
% std::vector<int> vector;
% // vector.push_back(element) - add in end
% // vector.pop_back() - remove last
% // vector[index] - get elem
% \end{lstlisting}
% \end{implementation}

% \subsection{Поддержка минимума на стеке и очереди}

% \textbf{Циклический буфер:}

% Это структура данных, использующая единственный буфер фиксированного размера таким образом, как будто бы после последнего элемента сразу же снова идет первый. С его помощью можно реализовать очередь на массиве. Так структура выйдет cache-friendly.

% \begin{implementation}{Циклический буфер}{}
%   \begin{lstlisting}[language=c++]
% // second homework c++))
% \end{lstlisting}
% \end{implementation}

% \textbf{Очередь на двух стеках:}
...
% \begin{definition}{Групповой анализ}{}
%   В ходе группового анализа исследователь показывает, что в наихудшем случае суммарное время выполнения последовательности всех $n$ операций равно $T(n)$. Поэтому в наихудшем случае средняя, или амортизированная, стоимость, приходящаяся на одну операцию, определяется соотношением $T(n)/n$.
% \end{definition}

% \textbf{Замечание:} Такая амортизированная стоимость применима ко всем операциям, даже если в последовательности имеется несколько разных их типов.

% \subsection{Бухгалтерский учет}

% \subsection{Метод потенциалов}

% \section{Вопрос 8}

% \subsection{RSQ — Range Sum Query}

% \begin{task}{Range Sum Query (RSQ)}{}
%   \textbf{Дано:} массив из $n$ элементов $A[0], A[1], \ldots, A[n-1]$.

%   \textbf{Запрос:} найти сумму элементов на отрезке $[l, r]$:
%   \[
%     \text{RSQ}(l, r) = \sum_{i=l}^{r} A[i] = A[l] + A[l+1] + \ldots + A[r]
%   \]
% \end{task}
...
%   \item $\text{RSQ}(2, 5) = 4 + 1 + 5 + 9 = 19$
%   \item $\text{RSQ}(0, 3) = 3 + 1 + 4 + 1 = 9$
% \end{itemize}

% \textbf{Применение:}
% \begin{itemize}
%   \item Вычисление суммы продаж за период
%   \item Подсчёт количества элементов на отрезке
%   \item Нахождение среднего значения на подмассиве
% \end{itemize}
...
% \subsection{Разреженная таблица (Sparse Table)}
...
% \subsection{Дерево отрезков (Segment Tree)}
...
% \end{lstlisting}
% \end{algorithm}
% \begin{algorithm}{Приватные поля}{}
%   \begin{lstlisting}[language=c++]
% private:
%   struct Node {
%     int value = 0;
